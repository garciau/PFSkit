get_HOMEPOS <- function(questionaire) {
  ...
  ## ¿Cuántas de las siguientes cosas hay en tu casa? - TV y smartphone
   q_15ST012_0_3 <- questionaire %>%
     select(stidstd, `15-ST012`, `15-ST012_3`) %>%
     rowwise() %>%
     mutate_at(vars(`15-ST012`, `15-ST012_3`),
               funs(TA = case_when(
                 . == 1 | . == 2 ~ 0, # ninguno | uno
                 . == 3 ~ 1, # dos
                 . == 4 ~ 2, # tres o más
  ...
  q_15ST013 <- questionaire %>%
    ...
    mutate_at(vars(contains('ST013')),
              funs(`15-ST013TA` = case_when(
                . == 1 ~ 5, # de 0 a 10 libros
                . == 2 ~ 4, # de 11 a 25 libros
                . == 3 ~ 3, # de 26 a 100 libros
                . == 4 ~ 2, # de 101 a 200 libros
                . == 5 ~ 1, # de 201 a 500 libros
                . == 6 ~ 0, # más de 500 libros
  ...
  print(xsi_fixed)
  print(b_fixed)
  A1<- designMatrices( resp=HOMEPOSrecoded%>%
                       select(-stidstd))$A
  ...
  return(HOMEPOS_idx %>% select(stidstd,HOMEPOS = theta )%>%
           mutate(HOMEPOS=(HOMEPOS-h_params$HOMEPOS$mean)/h_params$HOMEPOS$sd))
  #return(HOMEPOS_idx %>% select(stidstd,HOMEPOS = theta )%>%
  #                  mutate(HOMEPOS=(HOMEPOS-0.66)/0.53))
